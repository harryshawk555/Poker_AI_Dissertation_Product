...
def GameTestEnvironment():
    ...
    hand_finished = False
    AllIn = False
    absVal = preFlopScores[findAbsVal(preFlopScores, aHand)][1]
    ...
    while hand_finished != True:
        ...
        while action_finished != True and AllIn != True:
            ...
            if action == True and aStack>0:
                prevABet = pBetSize
                pbet, pBetSize = ai_decision(Ai_agent,aHand,Rboard,pot,betVal,(aStack+prevABet),absVal)
                if pBetSize > aStack:
                    pBetSize = aStack
                    pbet = 'AllIn'
                if pBetSize > bStack:
        ...
        if hand_finished == False:
            if len(Rboard) == 0:
                ...
                postFlopScores = handRankAiCalcsPostFlop(Rboard, Deck)
                absVal = postFlopScores[findAbsVal(preFlopScores, aHand)][1]               
            elif len(Rboard) == 3:
                board.pop(0)          
                Rboard.append(board[0])
                board.pop(0)
    ...
    Ai_agent.learn(outcome)        


arr = []
for x in range(1,2653):
    arr.append(x/2)
...
Greedy_Vals = pd.DataFrame.from_dict(Ai_agent.G)
Greedy_Vals.to_csv('greedyVals.csv')

listOutcomes = pd.DataFrame(outcomeList)
listOutcomes.to_csv('aiTestOutcomes.csv')
