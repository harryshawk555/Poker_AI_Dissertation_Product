...
def load_greedy():
    greedyDF = pd.read_csv('greedyVals.csv')
    greedyArr = greedyDF.to_numpy()
    greedyNum = np.empty((2652,6))
    for i in range(0,len(greedyArr)):
        for j in range(1,len(greedyArr[1])):
            greedyNum[i,j-1] = greedyArr[i][j]
    return greedyNum
...
class Agent(object):
    ...
    def learn(self,outcome):
        ...
        for state, next_move, reward in self.state_history:
            self.G[int(2*state)][next_move] = self.G[int(2*state)][next_move] + a*(target-self.G[int(2*state)][next_move])
    ...
    def choose_action(self, state, allowed_moves):
        ...
        if n<self.random_factor:
            ...
            maxG = -10e-15
            for i in range(0,6):
                if self.G[int(2*state)][i] == max(self.G[int(2*state)]):
                    next_move = i
            if self.G[int(2*state)][next_move] < maxG:
                next_move = 0              
        self.handHist.append((state,next_move))
        return next_move
    
    def choose_action_test(self, state):
        temp = self.G[int(2*state)]
        for i in range(0, len(temp)):
            if temp[i] == max(temp):
                move = i
                break
        self.handHist.append((state,move))
        return move
...
def ai_decision(ai_agent, hand, board, pot, betVal, stack, absVal):
    action = ai_agent.choose_action_test(absVal)
...
def ai_decision_test(ai_agent, pot, betVal, stack, absVal):
    action = ai_agent.choose_action_test(absVal[1])
